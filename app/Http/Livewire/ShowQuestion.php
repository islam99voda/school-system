...
class ShowQuestion extends Component
{
    ...
    {
        $stuDegree = Degree::where('student_id', $this->student_id) //to know if the student didnt take this exam before
        ...
        if ($stuDegree == null) {
            ...
            if (strcmp(trim($answer), trim($right_answer)) === 0) { // if the answer student equal right_answer
                $degree->score += $score;
            } else {
                $degree->score += 0;
            }
        ...
        if ($this->counter < $this->questioncount - 1) { // if not the last question
            $this->counter++; // go to next question
        } else {
