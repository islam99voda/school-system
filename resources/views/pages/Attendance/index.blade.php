        ...
        <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
            ...
            @foreach ($students as $student)
                <tr>
                    ...
                    <td>
                            @php
                                $attendance = $student->attendance()->where('attendence_date', date('Y-m-d'))
                                ->where('student_id', $student->id)->first();
                            @endphp
                            <label class="block text-gray-500 font-semibold sm:border-r sm:pr-4">
                                <input name="attendences[{{ $student->id }}]"
                                    {{ isset($attendance) ? 'disabled' : '' }}
                                    {{ isset($attendance)  && $attendance->attendence_status == 1 ? 'checked' : '' }}
                            ...
                            <label class="ml-4 block text-gray-500 font-semibold">
                                <input name="attendences[{{ $student->id }}]"
                                   {{ isset($attendance) ? 'disabled' : '' }}
                                   {{ isset($attendance)  && $attendance->attendence_status == 0 ? 'checked' : '' }}
